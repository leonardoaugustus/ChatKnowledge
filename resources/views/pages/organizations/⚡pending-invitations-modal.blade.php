...
new class extends Component {
    ...
    {
        if (session()->pull('organization-invitation-accepted')) {
            Flux::toast(variant: 'success', text: __('Invitation accepted.'));
        }
    }

    /**
     * @return \Illuminate\Support\Collection<int, array{code: string, inviter_name: string, organization_name: string}>
     */
    #[Computed]
    ...
    private function findPendingInvitation(string $code): OrganizationInvitation
    {
                ...
                <div class="grid gap-4">
                    @foreach ($this->pendingInvitations as $invitation)
                        <div data-test="pending-invitation-row" class="rounded-lg border border-zinc-200 p-4 dark:border-zinc-700">
                            <div class="space-y-1">
                                <p class="font-medium">{{ $invitation['organization_name'] }}</p>
