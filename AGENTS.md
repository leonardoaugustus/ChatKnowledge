...
- **Simple bar dashboards** MUST be built with plain Tailwind + Blade (a proportional-width `div` per bar), NOT a charting library. Most internal dashboards (usage per agent, questions per period, docs per status) are clearer this way and add zero dependencies.
...
- A user's `current_organization_id` MAY point to an organization they no longer belong to (e.g. removed). Reads MUST validate membership via the `organization_user` pivot before trusting it.

### The BelongsToOrganization trait + global scope

- Every model that holds tenant data (e.g. `Agent`, `Document`, `Conversation`, `UsageRecord`) MUST use the `App\Models\Concerns\BelongsToOrganization` trait.
- The trait MUST:
    - Add a global scope filtering by the active organization's id (`where('organization_id', ...)`).
    - Auto-fill `organization_id` on model creation from the active organization (`creating` hook).
...
- A middleware (e.g. `EnsureActiveOrganization`) MUST run on all authenticated, organization-scoped routes and verify the user belongs to their `current_organization_id` via the `organization_user` pivot.
...
### Cross-tenant safety (inviolable)
...
- For AI agents: each `Agent` has its own dedicated OpenAI vector store (`vector_store_id`). A query MUST confirm (a) the agent belongs to the active organization, then (b) only query that agent's vector store. Never share a vector store across organizations or agents.
...
- All AI calls MUST be wrapped in Service classes under `app/Services/` (e.g. `app/Services/Ai/`), never invoked directly from Components/Controllers — consistent with the slim-component rule.
...
- Domain enums (e.g. `DocumentStatus`, `AgentStatus`, `Role`) live in `app/Enums`, are cast on their models, and are used instead of raw strings everywhere.
